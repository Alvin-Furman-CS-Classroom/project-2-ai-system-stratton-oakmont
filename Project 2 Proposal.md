...
**Input:** News sentiment data retrieved from Alpha Vantage's Market News & Sentiment API, including: article sentiment scores (-1 to 1), sentiment labels (Bullish/Bearish/Neutral), article volume, topic distribution (earnings, economy, etc.), and sentiment trend over time.

**Output:** Predicted **market sentiment regime**:
- **Bullish:** Overall positive news sentiment, optimistic outlook
- **Bearish:** Overall negative news sentiment, pessimistic outlook
- **Neutral:** Mixed or low-confidence sentiment signals

Also outputs confidence score (how certain the prediction is), top headlines driving the classification, and recommends which evolved strategy from Module 3 historically performed best in the predicted sentiment regime.

**Integration:** This module bridges strategy discovery and execution using real-world news data. Different strategies perform better in different sentiment conditions—a momentum strategy might excel when sentiment is bullish but fail when sentiment is mixed. By classifying sentiment, the system picks the right tool for the job. The classifier is trained on historical sentiment features (average score, article volume, trend direction, topic mix) paired with known market outcomes, learning optimal feature weights through logistic regression.

**Prerequisites:** Supervised Learning (Week 7): Logistic regression, classification, training/test splits, feature engineering. Training labels derived from historical price movements following sentiment patterns.
...
| 4: Market Sentiment Classifier | Supervised Learning | Week 7 | CP3 (Mar 19) |
...
4. **Supervised Learning** — Market sentiment varies, and different strategies suit different conditions. A logistic regression classifier learns to recognize sentiment regimes from news data features (sentiment scores, article volume, trends). The model is trained on historical patterns where we know the subsequent market outcome, allowing it to predict future regimes from current sentiment signals.
